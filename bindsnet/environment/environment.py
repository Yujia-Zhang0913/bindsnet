...
class MuscleEnvironment(Environment):
    ...
    def __init__(self, name: str, encoder: Encoder = NullEncoder(), **kwargs) -> None:
        ...
        assert (self.eng is None), "Failed to connect with  matlab" # if not, exit
        self.eng.load_system('actuator.slx')  # load the model
        self.Joint_angle = []
        #
        # self.encoder = encoder
        #
    ...
    def step(self, a: int) -> Tuple[torch.Tensor, float, bool, Dict[Any, Any]]:
        ...
        """
        # Call simulink's environment to step
        self.eng.set_param("actuator", "SimulationCommand", "start", nargout=0)
        self.eng("actuator", "SimulationCommand", "step", nargout=0)
        self.eng("actuator", "SimulationCommand", "pause", nargout=0)
        # TODO due to the data structure of matlab workspace
        self.Joint_angle.append(self.eng.workspace["angle1"])
        self.preprocess()
